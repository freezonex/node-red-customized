...
var events = require('nr-test-utils').require(
...
function addNode(type, x, y) {
  ...
  browser.setValue(
    '//*[@id="red-ui-palette-search"]/div/form/input',
  ...
  browser.moveToObject(
    ...
    previousX + 300,
    previousY + 100
  ); // adjust to the top-left corner of workspace.
  browser.buttonUp();
  // Last node is the one that has been created right now.
  var nodeElement = browser.elements(
    '//*[contains(concat(" ", normalize-space(@class), " "), " red-ui-flow-node-group ")][last()]'
  );
...
function deleteAllNodes() {
  browser.waitForVisible(
    '//*[contains(@class, "active")]/a[@class="red-ui-tab-label"]'
  );
...
function deploy() {
  browser.call(function () {
    return when.promise(function (resolve, reject) {
      ...
      browser.clickWithWait('#-button-deploy');
    });
  });
  browser.waitForText('#-button-deploy', 10000);
